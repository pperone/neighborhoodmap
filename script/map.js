...
function AppViewModel() {
  ...
  this.locations = ko.observableArray([
    {title: 'Red Robin Gourmet Burgers', location: {lat: 47.750197, lng: -117.3991093}},
  ...
  this.filteredArray = ko.computed(function() {
    ...
    return ko.utils.arrayFilter(self.locations(), function(marker) {
      var match = marker.title.toLowerCase().indexOf(search) >= 0;
...
this.selectMarker = function (click) {
  self.clickedMarker(click);
  if (click != null) {
    self.clickedMarker().showMarkerInteraction();
  }
};
